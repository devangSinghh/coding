...
double median(vector<int>arr1, vector<int>arr2) {
	int n1 = arr1.size(), n2 = arr2.size(), l = 0, r = 2 * n2;
	while (l <= r) {
		int m2 = (l + r) / 2, m1 = n1 + n2 - m2;
		double l1 = m1 == 0 ? INT_MIN : arr1[(m1 - 1) / 2];
		double l2 = m2 == 0 ? INT_MIN : arr2[(m2 - 1) / 2];
		double r1 = m1 == 2 * n1 ? INT_MAX : arr1[m1 / 2];
		double r2 = m2 == 2 * n2 ? INT_MAX : arr2[m2 / 2];
		if (l1 > r2) l = m2 + 1;
		else if (l2 > r1) r = m2 - 1;
		else return (max(l1, l2) + min(r1, r2)) / 2;
	}
